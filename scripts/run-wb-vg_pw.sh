...
rm -Rf /mnt/hd/flash/zodiac;
mkdir -p /mnt/flash/zodiac;
#ln -s /mnt/hd/flash/zodiac /mnt/flash/zodiac;
...
NFS_CORES_DIR=$LOCAL_NFS_ROOT_DIR/cores
...
mkdir -p $NFS_CORES_DIR

ulimit -c unlimited
echo "$NFS_CORES_DIR/%e.%t.%p.core" > /proc/sys/kernel/core_pattern
...
build_oem_bins=$NFS_BINS_DIR/oem_snmp,$NFS_BINS_DIR/oem_cdl,$NFS_BINS_DIR/dsgcc,$NFS_BINS_DIR/dsgmanager
...
#    --suppressions=$VALGRIND_DIR/lib/valgrind/zodiac_mk.supp --suppressions=$VALGRIND_DIR/lib/valgrind/default.supp \
#    --undef-value-errors=no --leak-check=full --show-leak-kinds=definite,indirect  --workaround-gcc296-bugs=yes \
    $NFS_BINS_DIR/supervisor "$VALGRIND_DIR/bin/valgrind -v --run-libc-freeres=no --debuginfo-server=127.0.0.1:1500 --track-fds=yes --undef-value-errors=no --leak-check=full --show-leak-kinds=definite,indirect  --workaround-gcc296-bugs=yes $NFS_BINS_DIR/powerup-launcher"
    ;;
    * )
