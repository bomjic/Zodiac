...
source /bin/mbaikov/scripts/support_script.sh
...
#modified launch
build_oem_bins_no_bm=/bin/mbaikov/Stress/DCD_stress/Transmit_DCD_Cycle
...
case  "$configuration" in
    ...
    cd $NFS_BINS_DIR
    /bin/valgrind/bin/valgrind -v --run-libc-freeres=no \
    --debuginfo-server=127.0.0.1:1500 --track-fds=yes \
    --trace-children=yes --trace-children-skip=$system_bins,$system_oem_bins,$build_oem_bins_no_bm,$build_bins,$dalgroup_bins \
